...
public protocol SystemDisplayService {
    ...
    /// ⚠️ **已知缺陷,勿用于判断物理连接**——它数的是 framebuffer 的 `DisplayWidth`,
    /// 而屏一被禁用该键就消失,所以它**数不到被关掉的屏**。
    /// 下面那句「不受本 app 软件断开影响」是**错的**,2026-09-18 实测推翻:
    /// 两块外接屏、其中一块被本 app 关掉时,本方法报 1,而 `liveExternalCount()` 报 2(正确)。
    /// 曾据此把一块还连着的屏从菜单里藏起来,用户再也点不开。
    /// 需要物理连接真相的地方一律用 `liveExternalCount()`。本方法已无产品代码使用,待移除。
    ///
