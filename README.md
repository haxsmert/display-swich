...
而系统仍记着被关掉的内建屏(实测:断开状态不因拔线回滚)→ 全黑。
更糟的是这时无法自愈:本 app 其余的状态对账全挂在「菜单被打开」上,而全黑时根本点不开菜单栏。

所以 app 挂了一条**内核事件**监听(IOKit 服务终止通知,**零轮询**):拔线瞬间内核直接推送,
此时若「一块能亮的屏都不剩、而本 app 还关着屏」,就立刻把关掉的屏全开回来(失败会有界重试)。

> 判据与触发源**都不用 CoreGraphics**,这是实测逼出来的(2026-09-07 拔拓展坞现场):
> 拔线时它的重配置回调**全程 0 次派发**,活跃屏列表还报着拔线前的屏数;
> 连底层 framebuffer 计数也要**滞后约 3.5 秒**才归零。只有「带 EDID 的显示传输节点」
> 与内核通知同步、当场归零,所以判据用它。
>
> 也因此**没有采用定时轮询**:息屏时活跃屏数**会归零**,与全黑完全同形,轮询分不清二者,
> 会在你息屏时擅自把你关掉的屏开回来;而息屏不产生任何拔线内核事件。

救援路径写诊断日志到

`~/Library/Logs/DisplaySwitch.log`

——全黑时你看不到任何界面,万一救援失败,这是事后唯一能查清「通知到没到、判据是多少」的地方。
