...
核心规则:**绝不把用户置于"无法恢复的全黑"**。这需要**两层**,缺一不可——
第一层管「按下开关那一刻」,第二层管「那之后世界变了」。

### 第一层:关屏前的静态校验(`canDisable`)
...
静态校验只能保证**按下开关那一刻**不全黑,它默认「作兜底的那块活跃屏会一直在」。
可现实会打破这个前提:关掉内建屏用外接屏,然后**拔掉拓展坞**——兜底的屏被物理拔走,
而 WindowServer 仍记着被关掉的内建屏(实测:断开状态不因拔线回滚)→ 活跃屏归零 → 全黑。
更糟的是这时无法自愈:本 app 其余的状态对账全挂在「菜单被打开」上,而全黑时根本点不开菜单栏。

所以 app 注册了 `CGDisplayRegisterReconfigurationCallback`:**只要显示配置一变,就检查
「一块活跃屏都不剩、而本 app 还关着屏」,是则立刻把关掉的屏全开回来**(失败会有界重试)。

> 为什么是事件驱动、而**不是**定时轮询——两条都是实测(M 系列 · macOS 26):
> 息屏时 `CGGetActiveDisplayList` **确实会归零**(active=0 / online=1 / isAsleep=true,
> 头文件对 active 的定义本就是 "connected, **awake**, and available for drawing"),
> 所以轮询分不清「息屏」和「全黑」,会在你息屏时擅自把你关掉的屏开回来;
> 而息屏与唤醒**全程不产生**该回调(12 秒采样,回调 0 次),事件驱动天然免疫这个陷阱。
